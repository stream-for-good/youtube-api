...
final class AllCapturesOutputDataTransformer extends AbstractController implements CollectionDataProviderInterface,  RestrictedDataProviderInterface
{
    private $em;
    private $repoCapture;
    ...
    private function queryData(): array
    {
        ...
        $msg = [];
        $data = $this->repoCapture->findAll();
            
        $prefix = $this->const_prefix;

        foreach ($data as $value) {

            $logLink = $prefix . "/api/log/". $value->getLog()->getId();
            $logLinkArray = [
                'rel' => 'log_id',
                'href' => $logLink
            ];
            
            $captureLink = $prefix . "/api/capture/". $value->getId();
            $captureLinkArray = [
                'rel' => 'capture_id',
                'href' => $captureLink
            ];
            ...
            $links = array($captureLinkArray, $logLinkArray, $videoLinkArray);

                $dataSend = [
                    "captureID"=>$value->getId(),
                    "logID"=>$value->getLog()->getId(),
