...
final class AllCommentsOutputDataTransformer extends AbstractController implements CollectionDataProviderInterface,  RestrictedDataProviderInterface
{
    private $em;
    private $repoComment;
    ...
    private function queryData(): array
    {
        ...
        $msg = [];
        $data = $this->repoComment->findAll();
            
        $prefix = $this->const_prefix;

        foreach ($data as $value) {

            $commentLink = $prefix . "/api/comment/". $value->getId();
            $commentLinkArray = [
                'rel' => 'comment_id',
                'href' => $commentLink
            ];
            ...
            $links = array($commentLinkArray, $videoLinkArray);
